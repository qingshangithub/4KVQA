...
int main() {
	...
	string paraPath = "./model/parametersData.txt";
	...
	free(rescale_vector_4k_1);
	free(rescale_vector_4k_2);
	free(dct_data);

	time_t end = clock();
	printf("the running time is:%fs\n", double(end - start) / CLOCKS_PER_SEC); //̑
	cout << result << endl;
